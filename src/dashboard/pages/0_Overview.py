...
analyst_header(
    "Northern Signal — Overview",
    "Canadian bank systemic risk, translated into a governed portfolio decision.",
...
page_intro(
    ...
    how=(
        "Follow the grouped workflow in the sidebar: <b>Overview → Risk → Scenarios → Models → Decision</b>. "
        "Performance and Research preserve the supporting evidence without competing with the primary journey."
    ),
)
...
business_value_panel(
    ...
    intro=(
        "This platform is not trying to prove that an active model always beats Nasdaq or ZEB. "
...
with left:
    ...
    fig.add_hline(y=60, line_dash="dot", line_color="#ffb300", opacity=0.5, annotation_text="Moderate / Elevated", annotation_font_color="#ffb300")
    fig.add_hline(y=80, line_dash="dot", line_color="#f44336", opacity=0.5, annotation_text="Elevated / High", annotation_font_color="#f44336")
    fig.add_hline(y=90, line_dash="dot", line_color="#b71c1c", opacity=0.5, annotation_text="High / Severe", annotation_font_color="#ef9a9a")
    fig.update_layout(
...
with right:
    insight_card(
        f"Current risk regime: {regime['label']}",
        regime["summary"],
        status=regime["tone"],
    )
...
with tab4:
    ...
    st.markdown(
        """
        1. **Overview** establishes market conditions and the current risk regime.
        2. **Risk** connects the bank network with the composite score.
        3. **Scenarios** turns documented shocks into transmission and portfolio impacts.
        4. **Models** compares RL and CVaR responses and their validation evidence.
        5. **Decision** synthesizes the risk budget, allocation, reasons, and rebalance actions.
        6. **Performance** labels and compares simulated and paper results.
        7. **Research** keeps data, methodology, assumptions, and limitations accessible.
        """
    )
